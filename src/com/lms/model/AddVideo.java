...
public class AddVideo {
    ...
    public void InsertOraDB(String Title, String Author, String Isbn, String Director,String TrackingId) throws SQLException {
        Connection lms=null;
        PreparedStatement prpStmt=null;
        try{
            lms = Sel_DB.getConnection(Type_DB.Oracle_DB);
            String query     = "INSERT into LMS_VIDEO values (?,?,?,?,?)";
            prpStmt = lms.prepareStatement(query);

            prpStmt.setString(1,Title);
            prpStmt.setString(2,Author);
            prpStmt.setString(3,Isbn);
            prpStmt.setString(4,Director);
            ...
            if(status == 1) {
                String msgSuccess = "Video Details entry added Successfully. ";
                System.out.println(msgSuccess);
                popupMsg.updateMsg(msgSuccess,"Video information");
            }
            else {
                String msgNotSuccess = "Error while updating the Video Details";
                System.err.println(msgNotSuccess);
                popupMsg.updateMsg(msgNotSuccess,"Video information");
            }
